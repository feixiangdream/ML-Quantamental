...
for i in range(len(ret.columns) - 1):
    ...
    final = total.iloc[int(len(total) * 0.3):, :].copy()  # 去掉30%小市值
    # MKT
...
resultew = pd.DataFrame(columns=factoreq.columns[1:],
                        index=['ret', 't', 'exret', 't', 'capmret', 't', 'ff3ret', 't', 'ff30ret', 't'])
resultvw = pd.DataFrame(columns=factoreq.columns[1:],
                        index=['ret', 't', 'exret', 't', 'capmret', 't', 'ff3ret', 't', 'ff30ret', 't'])
rf = pd.read_csv('../Data/RF.csv')
...
factor = []  # 因子名称
for i in range(len(file)):
    factor.append(os.path.split(file[i])[-1][:-4])
for i in range(10):
